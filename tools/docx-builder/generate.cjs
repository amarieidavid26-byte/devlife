...
children.push(
  ...
  p("Suita de teste numără 37 de teste pytest distribuite în cinci fișiere tematice: `test_apply_fix.py` (8 teste pentru contract, validator, lifecycle preview-confirm-rollback), `test_biometric_classifier.py` (10 teste pentru toate cele cinci stări cognitive și pentru callback-ul de schimbare a stării), `test_fallback.py` (8 teste pentru răspunsurile pre-cached și pentru mock-ul biometric reproductibil), `test_server_smoke.py` (6 teste pentru endpoint-urile HTTP și pentru factory-ul de analyzer), `test_ws_flow.py` (5 teste pentru fluxul complet WebSocket de la connect la mesaje validate)."),
  p("Coverage-ul total este 48%, cu distribuția: modulele apply_fix la 94-100% (validatorul și contractul sunt path-uri critice de safety, complet acoperite), fallback_responses la 100%, persistence/db la 72%, server la 58%, biometric_engine la 38% (cea mai mare parte este cod de OAuth WHOOP care nu poate fi testat fără credențiale live), content_analyzer la 25% (logica de detectare risky e 100% testată, restul depinde de Claude API). Această distribuție reflectă o decizie deliberată: testăm logica pură și contractele, dar nu apelăm API-uri reale în CI pentru a evita dependența de credențiale. Path-urile către Claude și WHOOP sunt totuși protejate prin try/except cu fallback responses testate. Suita rulează în aproximativ 2.5 secunde pe un MacBook Air M-series."),
);
