...
children.push(
  ...
  h2("3.20. Rularea codului C++ în browser — JSCPP"),
  p("Pe lângă Python (Pyodide) și JavaScript (Web Worker nativ), editorul rulează acum și C++ direct în browser, fără compilator local și fără apel către un serviciu extern. Integrarea folosește JSCPP, un interpret C++ scris în întregime în JavaScript, împachetat de Vite ca module worker (`frontend/src/apps/runners/cppWorker.js`). La click pe Run, codul sursă este trimis worker-ului împreună cu stdin-ul, interpretat sincron sub o limită internă `maxTimeout: 5000ms` și returnat ca `{stdout, stderr, exit, ms}` — exact aceeași formă ca runner-ele Python și JavaScript. Worker-ul rulează izolat: dacă utilizatorul scrie o buclă infinită sau o eroare de runtime, doar firul worker-ului este afectat, nu UI-ul jocului."),
  p("Trade-off-ul ales conștient: JSCPP suportă un subset al C++ (header-ele `<iostream>`, `<cmath>`, `<cstdio>`, `<cstdlib>`, `<cstring>`, `<ctime>`, `<iomanip>`, `<cctype>`, plus clase, pointeri și aritmetică standard), dar nu containere STL ca `<vector>` sau `<string>`. Pentru programe stil olimpiadă (algoritmi, citire din stdin, calcul matematic) acoperirea este suficientă. Alternativa — clang compilat în WebAssembly (~30-50 MB download) — ar fi oferit suport STL complet, dar cu un UX inacceptabil la prima rulare. Codul sursă conține deja un comentariu explicit cu lista exactă a header-elor suportate, deci utilizatorul știe instant ce funcționează. Erorile de runtime (de exemplu dereferențierea unui pointer nul) sunt rutate prin același pipeline `run_error` → Ghost intervention ca pentru Python și JavaScript — diagnosticarea este uniformă indiferent de limbaj."),
  screenshot(27, "Editor cu welcome.cpp deschis, buton Run apăsat, output `sum = 31` în panoul de stdout."),

  h2("3.21. Boxa muzicală conectată la Spotify — PKCE OAuth + Web Playback SDK"),
  p("Boxa din cameră (sprite-ul de lângă birou) servește dublu rol: când utilizatorul nu este conectat la Spotify, click pe ea pornește/oprește un pad procedural sintetizat cu Web Audio API (oscilatori cu detune ușor, LFO, filtru lowpass — niciun fișier audio extern, totul generat dinamic). Când utilizatorul își conectează contul Spotify Premium din meniul Settings, aceeași boxă devine un control play/pause pentru contul personal: redarea continuă pe un device virtual numit „DevLife — room speaker\", vizibil și în aplicația Spotify oficială alături de telefon și desktop."),
  p("Autentificarea folosește OAuth 2.0 cu PKCE (Authorization Code + Proof Key for Code Exchange, RFC 7636) — alegerea corectă pentru un Single-Page Application care nu poate stoca secrete. Frontend-ul generează un `code_verifier` aleator de 64 de caractere și un `state` random anti-CSRF, calculează provocarea SHA-256 din verifier și redirecționează către accounts.spotify.com. La întoarcerea cu `?code=`, browser-ul schimbă codul pe access_token + refresh_token printr-un POST direct la `accounts.spotify.com/api/token`, fără ca backend-ul DevLife să atingă vreodată token-urile. Toate apelurile API (transfer playback, play/pause) reîmprospătează transparent token-ul la 401, iar slider-ul de volum și butonul Mute din Settings controlează simultan pad-ul procedural și volumul SDK-ului Spotify."),
  p("Design intenționat: dacă variabila `VITE_SPOTIFY_CLIENT_ID` nu este setată în `.env`, întreaga secțiune Spotify dispare din meniu și boxa rămâne pe pad-ul procedural — zero regresie pentru utilizatorii care nu doresc integrarea sau nu au Spotify Premium. Setup-ul complet (înregistrare app la developer.spotify.com, configurare redirect URI, completare client_id) este documentat în `docs/spotify-setup.md`."),
  screenshot(28, "Meniul Settings cu secțiunea SPOTIFY: status „Connected as Matei Vultur\" și buton Disconnect; alături, boxa pulsantă din cameră."),
);
...
children.push(
  ...
  bullet("JSCPP 2.0.9 — interpret C++ pur JavaScript pentru runner-ul in-browser (MIT)"),
  bullet("xterm.js + @xterm/addon-fit — emulator terminal in browser (MIT)"),
  bullet("watchfiles — watcher de file system pentru reload selectiv (MIT)"),
  ...
  bullet("Spotify Web Playback SDK + Web API — integrare opt-in, OAuth 2.0 cu PKCE, sub ToS Spotify"),
