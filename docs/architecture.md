...
DevLife este o aplicatie client-server cu doua moduri operationale (jocul sau IDE-ul real), conectata la doua surse externe (biometrice + AI) si la o persistenta locala.
...
- `sleep_mode`: activare/dezactivare cand banda e scoasa (BLE tacut) sau HR sta sub 50 mai multe cicluri
- `degraded_mode`: banner cand WHOOP API este indisponibil
- `app_focus_change`: broadcast al schimbarii aplicatiei active
- `whoop_connected` / `whoop_disconnected`: OAuth-ul WHOOP a reusit, respectiv contul a fost deconectat
...
- `keystrokes`: batch de perechi (interval, categorie) din KeystrokeCapture, max 300 pe mesaj
- `firewall_block`: terminalul raporteaza o comanda blocata (devine interventie, vezi mai jos)
- `run_error`: cod + eroare de la butonul Run, analizate imediat, fara sa treaca prin bucla `pending_content`
- `set_lang`: comuta limba ghost-ului (ro/en)
- `set_personality`: comuta personalitatea (coach/friend/sarcastic)
- `resume_live` si `wake`: iesirea din starea demo, respectiv trezitul personajului cand utilizatorul se misca
...
- `GET /api/session/replay`: seriile HR + interventii pe timeline comun (vezi subsistemul replay)
- `GET /api/session/report` + `GET /api/session/report/html`: raportul de sesiune (JSON / HTML exportabil)
- `GET /api/whoop/auth`: initiaza OAuth flow
- `GET /api/whoop/callback`: primeste codul OAuth si schimba pe token

Rutele privilegiate cer header-ul `X-DevLife-Token` (session token per-proces): `GET/POST /api/settings` (chei BYOK), `POST /api/whoop/disconnect`, `/api/files/*` si pornirea code-server-ului.

### Layer 3: FastAPI Backend (Python)

Locatie: root `*.py` + `apply_fix/` + `persistence/`

**Core orchestration**, impartit in 4 module:
- `runtime.py`: `AppState` dataclass (toate variabilele globale incapsulate), singleton-urile
  engine/brain/analyzer, `build_biometric_msg` si `broadcast_sync()`, care marshaleaza
  broadcast-urile din thread-uri non-async catre event loop cu `asyncio.run_coroutine_threadsafe`
...
- `RISKY_COMMAND_PATTERNS`: 11 pattern-uri regex pentru detectie instant (latenta < 1ms)
...
- Pastreaza ultimele 100 de analize Claude
- Summary-ul injectat in prompt-ul urmator se construieste din ultimele 10
...
Contractul, regulile de safety si audit-ul stau in fisiere separate:
- `contract.py`: `PatchContract` Pydantic cu file, language, range, replacement_text, rationale, severity, original_text
- `validator.py`: reguli concrete (rationale non-vid, end_line >= start_line, range <= 500 linii, replacement <= 500 linii, fara shell metacharacters)
...
- `connect()`: singleton conexiune cu PRAGMA `journal_mode=WAL` (concurrency read non-blocking)
...
Schema are 8 tabele, construite din 4 migratii. `001_init.sql` creeaza primele 6:
...
- `002_replay.sql`: adauga coloana `state` pe `biometric_samples` (colorarea timeline-ului de replay), fara tabel nou
- `003_calibration.sql`: tabelul `calibration` (key, value, updated_at), baseline-urile personale persistente
- `004_settings.sql`: tabelul `settings` (key, value, updated_at), cheile API configurate din aplicatie (BYOK)
...
`test_keystroke_dynamics.py` (13 teste).
...
`biometric_loop` persista un sample la fiecare ciclu (HR, HRV cu valoarea live daca exista,
...
deseneaza ca polilinie HR colorata dupa starea cognitiva, cu tick-uri rosii pentru
interventii. Scrub cu mouse-ul arata valorile punctuale. Rezultatul: demo-ul nu mai e o
simulare, e o **dovada pe date reale** ("aici am obosit, aici ghost-ul m-a oprit din force push").

### subsistem: raport de sesiune exportabil

`db.get_session_report()` agrega o sesiune: distributia starilor din sample-uri (fiecare
sample ≈ un ciclu de 5s, deci numaratoarea pe stare ≈ timp petrecut), plus min/max/mediu pe
puls si HRV si lista interventiilor. `session_report.py::build_report_html` randeaza totul
intr-un singur fisier HTML **self-contained** (CSS si SVG inline, zero dependinte externe)
care se deschide offline si se poate trimite pe mail. Timeline-ul SVG coloreaza segmentele
de puls dupa starea cognitiva si marcheaza interventiile cu tick-uri. Culorile sunt o copie
...
`vscode-extension/` vorbeste exact acelasi protocol WebSocket ca jocul, fara nicio linie de
...
pentru endpoint-urile privilegiate, iar extensia foloseste doar endpoint-ul de joc,
neprivilegiat. Asta muta DevLife din "joc cu editor" in "utilitar care functioneaza in
editorul tau real".
...
si sa te gandesti, nu ca sa te blocheze din propria masina; decizia ramane insa
...
SQLite-ul local, deci cheile raman pe masina utilizatorului. Asta pastreaza povestea
...
|---------|-------------|------------|
| Vanilla JS + PixiJS | React + Three.js | control complet pe render-ul izometric procedural la ~50KB minified; React ar fi adaugat doar overhead de lifecycle |
| FastAPI | Flask, Django, Express | Pydantic + WebSocket native + async/await + auto-validation 422 |
| SQLite | PostgreSQL | portabil, zero deployment overhead; WAL acopera concurrency la volumul nostru (~500 inserts/h) |
...
| AppState dataclass | global variables | testabilitate (fixture) si type hints; tot state-ul global sta incapsulat intr-un singur loc |
| Patch validator separat de orchestrare | inline checks | testabil unitar si reutilizabil; regulile de safety nu se amesteca cu orchestrarea |
| Apply Fix audit in DB | log files | query-abil si durabil dupa restart; se vede si din admin tools |
| Claude API cu timeout 15s | timeout default al SDK-ului (10 min) | previne blocarea ghost_loop daca API atarna |
| WS bounds explicite | Pydantic global | WS nu suporta Pydantic decoder; bounds inline sunt mai rapide |

## paradigme de programare

- **Functional** in `persistence/db.py` si `apply_fix/validator.py`: functii pure, cu input
  si output explicite; fara state ascuns, deci testabile izolat.
- **Obiectual** pentru `BiometricEngine`, `GhostBrain`, `ContentAnalyzer`, `ScreenCapture`:
  clasele incapsuleaza state (history, baseline, tokens) si policies (cooldowns), cu
  side-effects controlate in metode.
- **Dataflow** intre layer-uri: `AppState` si mesajele WebSocket tipizate curg unidirectional.
- **Event-driven**: `on_state_change_callback` si mesajele WS dau reactivitate fara polling.

## extensibilitate

Unde intervii ca sa adaugi ceva nou, fara sa atingi restul:

- app type nou: `APP_PROMPTS` in `content_analyzer.py`, `GAME_APPS` in `config.py` si un fisier nou in `frontend/src/apps/` dupa pattern-ul existent.
- stare cognitiva noua: `MockBiometrics.PRESETS`, `bio.classify()`, `GhostBrain.PROMPTS`, `fallback_responses.FALLBACKS` (+ `FALLBACKS_RO`).
- risky pattern nou: o singura linie in `RISKY_COMMAND_PATTERNS`.
- camp biometric nou: `MockBiometrics.PRESETS`, `BiometricEngine.fetch_data()`, `build_biometric_msg()`.
- scena noua: implementezi interfata `Scene` (enter, exit, update) si o inregistrezi in `SceneManager`.

Niciuna dintre extinderi nu modifica cod existent, doar adauga.
