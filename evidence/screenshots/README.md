...
- Numire: doar numarul din tabel (`01.png`, `02.png`, etc.); slug-ul descriptiv ramane in tabelul de status
...
- **ce arata**: terminal in-game cu comanda `git push --force` tastata + speech bubble Ghost cu "FATIGUE FIREWALL -- HRV 28ms, stres 1.8/3.0. 'force push' e ireversibil." + butoane "Save Draft" / "Do It Anyway" / "Remind Later"
- **cum**:
...
- **ce arata**: toast-ul de warning din partea de sus, titlu "Mod degradat", subtitlu despre modul demo offline cu date simulate
...
- **cum**: triggereaza DemoMode complet (din MainMenu → DEMO buton) si captureaza un cadru cu credits
...
- **ce arata**: DB Browser for SQLite cu cele 8 tabele expandate (sessions, interventions, biometric_samples, feedback, apply_fix_audit, consent, calibration, settings)
- **cum**:
  ```bash
...
- **focus**: 8 tabele, relatii (FOREIGN KEY), tipuri de date
...
- **ce arata**: tab C++ sau Go, butonul Run ascuns, hint-ul "Doar review AI" care cere comutarea pe PY/JS pentru a rula
...
- **Pentru a forta o stare**: apasa `1` (DEEP_FOCUS), `2` (STRESSED), `3` (FATIGUED), `4` (RELAXED), `5` (WIRED); ordinea vine din MockBiometrics.PRESETS, iar preset-ul de pornire e 4 (RELAXED)
- **Screenshot-ul de Railway (22)**: fa-l doar daca deploy-ul real e activ. Daca nu e, treci la 23 sau captureaza `docs/deploy-runbook.md` ca dovada de configurare.

## status pe fiecare captura
